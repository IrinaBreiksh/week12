...
const newText = text.slice(28,50);
...
const modifiedText = replacedText3.toUpperCase().slice(0,1) + replacedText3.toLowerCase().slice(1);
...
const buttonCalc = document.getElementById('button');
...
function calculateBMI(){
    ...
    const result = Math.round(weight/Math.pow(heightInMetres, 2));

    const resultCalculated = document.querySelector('#calc-result');
    const newParagraph = document.createElement('p');
    newParagraph.textContent = `Ваш индекс массы тела равен ${result}`;
    
    resultCalculated.append(newParagraph);
}
